    ...
    context.subscriptions.push(commands.registerCommand('mutica.run', () => {
        ...
        if (activeEditor && activeEditor.document.languageId === 'mutica') {
            ...
            if (compilerPath) {
                ...
                window.showErrorMessage(
                    'Mutica compiler not found. Please build it using "cargo build --release" in the Mutica project ' +
                    'or configure the path in settings (muticaLsp.compilerPath).'
                );
            }
        } else {
